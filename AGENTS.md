...
收盘提示词与早报对齐：同样是一至七条九段 + 第八条排版 + 第九条知识星球文案。数据截止北京时间 17:10。第五条写**当日**复盘（含龙虎榜，只映射固定板块），第六条检索窗是当日 08:30 至 17:10，并对照同日早报里指向当天的判断。

文件名：`archive/年/月/A股收盘报告_YYYY-MM-DD_1710.html`。仪表盘粘贴：`prompts/close-automation-dashboard-prompt.md`。版本见 `prompts/close-VERSION`。生成后同样运行 `python3 scripts/build_index.py`。`latest.html` 仍跳最新早报，收盘走 `latest-close.html`。日程 `10 9 * * 1-5`（北京时间工作日 17:10），模型用 GPT。
