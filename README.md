...
archive/年/月/A股收盘报告_YYYY-MM-DD_1710.html
```
...
建议新建自动任务，名称「A股收盘报告」，日程 `10 9 * * 1-5`（北京时间工作日 17:10），模型用 GPT，接上同花顺 iFinD。数据截止 17:10。当日复盘含龙虎榜，只写与固定板块或早报判断有关的上榜股。不要把收盘提示词贴进「A股开盘前多源晨报」。`latest.html` 仍然只打开最新早报；收盘报告用 `latest-close.html`。
...
之后在本机目录双击「生成收盘报告.bat」，会从 GitHub Pages 下载完整收盘 HTML 并打开。需要 Python 3。收盘自动任务 17:10 跑完并进入 `main` 之前，这个 BAT 不会编造报告。
